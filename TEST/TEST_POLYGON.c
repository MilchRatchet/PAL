...
void TEST_POLYGON_QUICKSTAR() {
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    VLL_CONTAINER_ADDPOINT(&container, VLL_UTILS_GETPOINT(p.x, p.y, 4, VLL_UTILS_COLOR(0xffffff)));
  }
...
void TEST_POLYGON_MONTRIANGLE() {
  TEST_PRINT_INPROGRESS("CGL_POLYGON_MONTRIANGLE");
  ...
  CGL_Polygon_t polygon      = CGL_POLYGON_MONOTONE(points, count);
  CGL_Polydiagonals_t result = CGL_POLYGON_MONTRIANGLE(&polygon, points, count);
  ...
  for (unsigned int i = 0; i < result.count; i++) {
    VLL_CONTAINER_ADDLINE(
      &container,
      VLL_UTILS_GETLINE(
        container.points[result.vertex1[i]], container.points[result.vertex2[i]], VLL_UTILS_COLOR(0x555555)));
  }
  ...
  TEST_PRINT_RESULT(success, "CGL_POLYGON_MONTRIANGLE");

  VLL_SCREEN_DESTROY(&screen);
  VLL_CONTAINER_DESTROY(&container);
  CGL_POLYGON_FREE(&polygon);
  CGL_POLYGON_DIAGONALSFREE(&result);
  free(points);
}
